...
| Termius's semantic chrome colours all have theme tokens | The bundled `themes/sshdeck.json` has no token for `--border-basic` (`#8d91a540`) or `--border-light` (`#8d91a51a`), and its `list.hover.background` equals the entity card background (`#282b3d`) instead of `--list-hover` (`#3e4257`). `sshdeck-app/src/main.rs` hardcodes those recovered values; prefer a token if the theme gains one. `rgba(0xRRGGBBAA)` parses as RGBA (`gpui-pre` `color.rs`), so `rgba(0x8d91a540)` is `8d/91/a5` at alpha `40`. |
| Theme `font.size` is the body text size | The bundled theme sets `font.size: 13` while the recovered `--default-font-size` is 14px. The chrome sets `.text_size(px(14.))` on its root; drop the override once the theme's `font.size` is 14. |
