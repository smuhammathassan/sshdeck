...
| A rounded frameless window can be requested through `WindowOptions` | No corner-radius field exists. `appears_transparent: true` hides the system title bar and draws content under it while keeping the native traffic lights (the closable/minimizable/resizable style masks are only set when `titlebar` is `Some`), so the close button works. `traffic_light_position: Some(point(..))` is a real field that positions the lights (gpui-component's `TitleBar` uses `(9, 9)` for its 34px bar); `None` means the platform default, not "hidden". Rounded corners are not expressible: gpui-component `window_border.rs` hardcodes `BORDER_RADIUS = 0` because GPUI clips to a rectangular content mask, so only the native macOS window radius applies (square elsewhere). Mark the header `WindowControlArea::Drag` (or set `app_owns_titlebar_drag`) so a hidden title bar still drags the window. Verified in `gpui-pre 0.3.5` (`platform.rs`, `elements/div.rs`) and `gpui-component 0.6.1` (`window_border.rs`, `title_bar.rs`). |
| Termius's semantic chrome colours all have theme tokens | **Resolved.** Dark already carries them: `border` (`#8d91a540`) is `--border-basic`, `sidebar.border` / `title_bar.border` (`#8d91a51a`) are `--border-light`, `table.row.border` (`#8d91a50d`) is `--border-extra-light`, and `list.hover.background` is `#3e4257` (`--list-hover`). `main.rs` wires the pane-tab hairline, the settings hairline, and the host-details separators through `cx.theme().sidebar_border`; the header keeps fixed dark hexes as the documented always-dark-header exception (see `render_header`). `rgba(0xRRGGBBAA)` parses as RGBA (`gpui-pre` `color.rs`), so `rgba(0x8d91a540)` is `8d/91/a5` at alpha `40`. |
| Theme `font.size` is the body text size | **Resolved.** The bundled theme sets `font.size: 14`, matching the recovered `--default-font-size` (14px); the root `.text_size(px(14.))` override is gone. |
| `gpui_kit::assets::IconName` is the complete Lucide catalog, so any Lucide glyph can be named | The enum is complete, but the asset source `main.rs` installs (`gpui_kit::assets::Assets` via `.with_assets(..)`) embeds **only** the ~100 icons listed in `gpui-kit-assets`'s `default-icons.txt`, and `AssetSource::load` returns `Err` for every other path. A full-catalog variant such as `Key`, `Fingerprint`, `Usb`, `LayoutGrid` or `List` therefore renders as nothing. Either install `gpui_kit::assets::AllAssets`, or draw the glyph from raw SVG bytes with `Icon::default().data(br#"<svg …>…</svg>"#)` — `Icon::data` is a real public builder (`gpui-component 0.6.1` `icon.rs`) and bypasses the asset source entirely. Verified in `gpui-kit-assets 0.6.1` (`build.rs`, `native_assets.rs`) and `gpui-component 0.6.1` (`icon.rs`). |
